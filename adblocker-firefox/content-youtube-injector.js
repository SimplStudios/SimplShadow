...
// Simple, reliable approach - inject IMA shim and ad data stripper
...
(function() {
  ...
  function injectScripts() {
    ...
    // Inject inline script for page context
    const script = document.createElement('script');
    script.textContent = `(function(){
'use strict';
if(window._simplshadowYT)return;
window._simplshadowYT=true;

// ========== IMA SDK SHIM ==========
// Fake Google IMA SDK that skips ads instantly
...
class EventHandler{
  ...
  _dispatch(e){
    for(const fn of(this._l.get(e.type)?.values()||[]))try{fn(e)}catch(x){}
  }
}

class AdDisplayContainer{
  constructor(c){if(c){const d=document.createElement('div');d.style.display='none';c.appendChild(d)}}
  destroy(){}
  initialize(){}
}
...
const AdEventType={
  ...
  VIDEO_CLICKED:'videoClicked',VIEWABLE_IMPRESSION:'viewable_impression',
...
const currentAd={
  ...
  getDealId:()=>'',getDescription:()=>'',getDuration:()=>0,getHeight:()=>0,
  getMediaUrl:()=>null,getMinSuggestedDuration:()=>0,getSkipTimeOffset:()=>-1,
  getSurveyUrl:()=>null,getTitle:()=>'',getTraffickingParameters:()=>({}),
  getTraffickingParametersString:()=>'',getUiElements:()=>[],
  getUniversalAdIdRegistry:()=>'unknown',getUniversalAdIds:()=>[],
  getUniversalAdIdValue:()=>'',getVastMediaBitrate:()=>0,getVastMediaHeight:()=>0,
  getVastMediaWidth:()=>0,getWidth:()=>0,getWrapperAdIds:()=>[],
  getWrapperAdSystems:()=>[],getWrapperCreativeIds:()=>[],isLinear:()=>true,
  isSkippable:()=>true
};
...
class AdsManager extends EventHandler{
  ...
  start(){
    requestAnimationFrame(()=>{
      const seq=[AdEventType.LOADED,AdEventType.STARTED,AdEventType.COMPLETE,AdEventType.ALL_ADS_COMPLETED,AdEventType.CONTENT_RESUME_REQUESTED];
      for(const t of seq)try{this._dispatch(new AdEvent(t,currentAd))}catch(e){}
    });
  }
  stop(){}
...
class CompanionAd{getAdSlotId(){return''}getContent(){return''}getContentType(){return''}getHeight(){return 0}getWidth(){return 0}}
...
class UniversalAdIdInfo{getAdIdRegistry(){return''}getAdIdValue(){return''}}
...
console.log('[SimplShadow] YouTube ad interceptor loaded');
})();`;
