...
class SimplShadowPopup {
  ...
  async loadState() {
    try {
      const result = await browser.storage.local.get(['shadowBlockState', 'whitelist', 'settings']);
  ...
  async togglePower() {
    ...
    try {
      await browser.storage.local.set({ enabled: this.state.enabled });
  ...
  async toggleWhitelist() {
    ...
    try {
      ...
      await browser.runtime.sendMessage({ action: 'updateWhitelist', whitelist });
      ...
      if (this.state.tabId) {
        browser.tabs.reload(this.state.tabId);
      }
    } catch (e) {
  ...
  async refreshPage() {
    if (this.state.tabId) {
      await browser.tabs.reload(this.state.tabId);
      window.close();
    }
  }
  ...
  async handleSettingToggle(e) {
    ...
    if (setting === 'useLessRam') {
      await browser.runtime.sendMessage({ action: 'setLowMemoryMode', enabled: value });
    } else if (setting === 'blockWebsockets') {
      await browser.runtime.sendMessage({ action: 'setWebsocketBlocking', enabled: value });
    } else if (setting === 'blockWebrtc') {
      await browser.runtime.sendMessage({ action: 'setWebrtcBlocking', enabled: value });
    } else if (setting === 'cosmetic') {
      await browser.runtime.sendMessage({ action: 'setCosmeticFiltering', enabled: value });
    }
  }
  
  async saveSettings() {
    try {
      await browser.storage.local.set({ settings: this.state.settings });
    } catch (e) {
